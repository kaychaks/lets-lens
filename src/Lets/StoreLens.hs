...
modify ::
  ...
  -> a
  -> a
modify (Lens r) f a = setS (r a) . f $ getS $ r a
...
fmodify ::
  ...
  -> a
  -> f a
fmodify (Lens r) f a = let Store s g = r a
                       in s <$> f g
...
mapL k = Lens $ \m -> Store (maybe (Map.delete k m) (\b -> Map.insert k b m)) (Map.lookup k m)
...
setL ::
  ...
  k
  -> Lens (Set k) Bool
setL k = Lens $ \m -> Store (bool (Set.delete k m) (Set.insert k m)) (Set.member k m)
...
compose ::
  ...
  -> Lens a b
  -> Lens a c
compose (Lens f) (Lens g) = Lens (\a -> let Store f1 r1 = g a
                                            Store f2 r2 = f r1
                                        in Store (f1 . f2) r2)
...
product ::
  ...
  -> Lens c d
  -> Lens (a, c) (b, d)
product (Lens f) (Lens g) = Lens $ \(a , c) -> let Store s1 a1 = f a
                                                   Store s2 a2 = g c
                                                in Store (\(b , d) -> (s1 b, s2 d)) (a1, a2)
...
choice ::
  ...
  -> Lens b x
  -> Lens (Either a b) x
choice (Lens f) (Lens g) = Lens $ \e -> case e of
  Left a  -> mapS Left (f a)
  Right b -> mapS Right (g b)
